...
type Selector func(nml.Node) bool

// hasChildMatch returns whether n has any child that matches a.
func hasChildMatch(n nml.Node, a Selector) bool {
...
func hasDescendantMatch(n nml.Node, a Selector) bool {
	for c := n.GetFirstChild(); c != nil; c = c.GetNextSibling() {
		if a(c) || (c.GetType() == nml.ElementNode && hasDescendantMatch(c, a)) {
			return true
		}
	}
	return false
...
func (s Selector) MatchAll(n nml.Node) (result []nml.Node) {
	if s(n) {
...
func (s Selector) Match(n nml.Node) bool {
	return s(n)
}

// Filter returns the nodes in nodes that match the selector.
func (s Selector) Filter(nodes []nml.Node) (result []nml.Node) {
	for _, n := range nodes {
		if s(n) {
			result = append(result, n)
		}
...
func typeSelector(tag string) Selector {
	...
	return func(n nml.Node) bool {
		return n.GetType() == nml.ElementNode && n.GetData() == tag
	}
}
...
func writeNodeText(n nml.Node, b *bytes.Buffer) {
	switch n.GetType() {
	case nml.TextNode:
		b.WriteString(n.GetData())
	case nml.ElementNode:
		for c := n.GetFirstChild(); c != nil; c = c.GetNextSibling() {
			writeNodeText(c, b)
		}
	}
...
func nodeText(n nml.Node) string {
	var b bytes.Buffer
	writeNodeText(n, &b)
	return b.String()
}
...
// children of n.
func nodeOwnText(n nml.Node) string {
	var b bytes.Buffer
	for c := n.GetFirstChild(); c != nil; c = c.GetNextSibling() {
		if c.GetType() == nml.TextNode {
			b.WriteString(c.GetData())
		}
	}
	return b.String()
}
...
func nthChildSelector(a, b int, last, ofType bool) Selector {
	return func(n nml.Node) bool {
		...
		i := -1
		count := 0
		for c := parent.GetFirstChild(); c != nil; c = c.GetNextSibling() {
			if (c.GetType() != nml.ElementNode) || (ofType && c.GetData() != n.GetData()) {
				continue
			}
			count++
			if c == n {
...
func emptyElementSelector(n nml.Node) bool {
	if n.GetType() != nml.ElementNode {
		return false
	}
	...
	for c := n.GetFirstChild(); c != nil; c = c.GetNextSibling() {
		switch c.GetType() {
		case nml.ElementNode, nml.TextNode:
			return false
		}
	}
...
func childSelector(a, d Selector) Selector {
	return func(n nml.Node) bool {
		return d(n) && n.GetParent() != nil && a(n.GetParent())
	}
}
...
func siblingSelector(s1, s2 Selector, adjacent bool) Selector {
	return func(n nml.Node) bool {
		...
		if adjacent {
			if n.GetPrevSibling() == nil {
				return false
			}
			return s1(n.GetPrevSibling())
		}

		// Walk backwards looking for element that matches s1
		for c := n.GetPrevSibling(); c != nil; c = c.GetPrevSibling() {
			if s1(c) {
				return true
			}
		}
